  ...
  return (
    <div className="col-start-3 col-end-4 row-start-2 row-end-4 flex flex-col items-center justify-end gap-8">
      <div className="relative flex items-center gap-2">
        ...
        />
        <p className="z-10 rounded-full bg-black/50 px-2 text-xl font-bold">
          x {me.score}
        </p>
        ...
        <AnimatePresence>
          {scoreDiffs.map(({ id, diff }) => (
            <motion.div
              ...
              className={`pointer-events-none absolute top-0 right-0 z-0 text-3xl font-bold drop-shadow-md ${diff > 0 ? "text-green-500" : "text-red-500"}`}
            >
              {diff > 0 ? "+" : ""}
              {diff}
            </motion.div>
      ...
      <div className="flex w-full flex-col items-end justify-end gap-4">
        <div
          className={`grid w-full grid-rows-[1fr] opacity-100 transition-all duration-300 ease-in-out`}
        >
